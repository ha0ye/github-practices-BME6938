...
* To start version control for a project folder, use the shell to navigate into the folder and run the command:
...
* each commit is created *manually*
  - `git add` files with changes (including new files)
...
* You can view the project state by using the `git checkout` command and the desired hash
...
When you have commits in one location that need to be synchronized to another location, there are 2 commands to use on a local repository:
...
* Don't be afraid to ask for help or look for workflow helpers (e.g. shell auto-completion)
